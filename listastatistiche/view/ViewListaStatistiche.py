...
class ViewListaStatistiche(QWidget):
    def __init__(self, parent=None):
        super(ViewListaStatistiche, self).__init__(parent)
    ...
    # Visualizzazione della statistica corrente selezionata
    def show_statistica(self, anno):
        if len(self.list_view.selectedIndexes()) > 0:
            selected = self.list_view.selectedIndexes()[0].row()
            statistica_selezionata = self.controller.get_statistica_by_index(selected)
            print("QUI"+sys.executable)
            self.vista_statistica = ViewStatistica(statistica_selezionata, selected, anno)
            self.vista_statistica.show()

    # Visualizzazione della lista di tutte le statistiche
    def filter_all(self):
    ...
    # Visualizzazione della lista delle statistiche dei soli prodotti
    ...
    # Visualizzazione della lista delle statistiche dei soli fornitori
    ...
    # Procedura del filtraggio che viene avviata subito dopo aver cliccato il bottone apposito
    ...
    # Metodo che informa l'utente dell'errato inserimento dell'anno. E' necessario inserire i dati corretti
    def popup_error_formato_data(self):
        msg = QMessageBox()
        msg.setWindowTitle("WARNING")
        msg.setText(
            "Anno inserito non valido.\n Inserire valore corretto nel seguente formato YYYY.")
        msg.setIcon(QMessageBox.Warning)
        time.sleep(0.3)
        msg.exec_()

    # Metodo che informa l'utente dell'errato tipo dell'inserimento dell'anno. 
    def popup_error_data(self):
        msg = QMessageBox()
        msg.setWindowTitle("WARNING")
        msg.setText(
            "Tipo di valore inserito non consentito.\n Inserire valore del tipo corretto.")
        msg.setIcon(QMessageBox.Warning)
        time.sleep(0.3)
        msg.exec_()
