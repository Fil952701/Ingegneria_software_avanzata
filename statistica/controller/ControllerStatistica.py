...
class ControllerStatistica():
    ...
    # metodi per andare a fare GET sui dati
    def get_nome(self):
    ...
    # metodi per andare a fare SET sui dati
    ...
    # Metodo per richiamare la statistica da calcolare in base alla scelta effettuata in stile switch case
    def smistatore_statistica(self, smistatore, anno):
        if smistatore == 0:
            return self.ordinamento_decrescente_prod(self.costruzione_dizionario(anno))
        elif smistatore == 1:
            return self.ordinamento_crescente_prod(self.costruzione_dizionario(anno))
        elif smistatore == 2:
            return self.ordinamento_decrescente_prod(self.prod_piu_redditizi(anno))
        elif smistatore == 3:
            return self.ordinamento_decrescente_forn(self.forn_piu_pagati(anno))
        elif smistatore == 4:
            return self.ordinamento_decrescente_forn(self.forn_da_cui_acquistiamo_di_piu(anno))
        elif smistatore == 5:
            return self.ordinamento_crescente_forn(self.forn_piu_rapidi_nella_consegna(anno))
        elif smistatore == 6:
            return self.andamento_finanziario(anno)

    # Metodo per costruire un dizionario contentente il codice del prodotto associato al numero di prodotti venduti
    def costruzione_dizionario(self, anno):
        ...
        for prodotto in lista_prodotti:
            ...
            elif "Venduto" in prodotto.stato and anno in prodotto.data_vendita:
                if prodotto.data_vendita is not None and "," in prodotto.data_vendita:
                    dizionario[prodotto.cod_prodotto] += 2
                elif prodotto.data_vendita is not None and "," not in prodotto.data_vendita:
                    dizionario[prodotto.cod_prodotto] += 1
    ...
    def prod_piu_redditizi(self, anno):
        lista_prodotti = self.controller_listaprodotti.get_lista_prodotti()
        dizionario = {}
    ...
    def forn_da_cui_acquistiamo_di_piu(self, anno):
        lista_ordini = self.controller_listaordini.get_lista_ordini()
        dizionario = {}
        ...
        for ordine in lista_ordini:
            if anno == "":
                dizionario[ordine.cod_fornitore] += ordine.strumenti_totali
            elif anno in ordine.data_arrivo_prevista:
                dizionario[ordine.cod_fornitore] += ordine.strumenti_totali
    ...
    def forn_piu_rapidi_nella_consegna(self, anno):
        lista_ordini = self.controller_listaordini.get_lista_ordini()
        dizionario = {}
        ...
        for ordine in lista_ordini:
            if ordine.data_arrivo_effettiva is not None:
                if anno is None:
    ...
    def andamento_finanziario(self, anno):
        diz_prod_vend = self.costruzione_dizionario(anno)
    ...
    def get_data_inizio_contratto(self, utente):
        if utente.data_inizio_contratto is None:
            ...
            mese = data_divisa[1]
            anno = data_divisa[2]
            data_convertita = anno + "-" + mese + "-" + giorno
