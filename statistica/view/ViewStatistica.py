...
import listastatistiche.view.ViewListaStatistiche


class ViewStatistica(QWidget):
    def __init__(self, statistica, selected, anno, parent=None):
        super(ViewStatistica, self).__init__(parent)
        ...
        if self.controller_stat.get_quantita() is not None and self.anno != "":
            ...
            label_nome = QLabel(self.controller_stat.get_nome() + ": " + str(self.anno))
            label_nome.setAlignment(QtCore.Qt.AlignCenter)
    ...
    def update_ui_stat(self, lista_ordinata):
        ...
        if not lista_ordinata:
            ...
            if self.selected == 0:
                ...
                self.creazione_ct("Nome", "Stato", "Strumenti totali")
    ...
    def update_ui_af(self, dizionario_af):
        ...
        plt.savefig("listastatistiche/data/grafico.jpg")
        plt.show()
    ...
    def andamento_finanziario(self, dizionario_af):
        ...
        if cont == 4:
             self.nessun_elem = QLabel()
             self.nessun_elem.setText("Nessun dato disponibile da visualizzare...")
             font = self.nessun_elem.font()
             font.setPointSize(18)
             self.nessun_elem.setFont(font)
             self.v_layout.addWidget(self.nessun_elem)
            ...
            pixmap = QPixmap("listastatistiche/data/grafico.jpg")
            self.immagine.setPixmap(pixmap)
            self.v_layout.addWidget(self.immagine)

    # Metodo che in base alla statistica scelta mostra una view differente
    def smistatore_viste(self):
        if self.selected == 0:
            self.update_ui_stat(self.controller_stat.smistatore_statistica(self.selected, self.anno))
        elif self.selected == 1:
            self.update_ui_stat(self.controller_stat.smistatore_statistica(self.selected, self.anno))
        elif self.selected == 2:
            self.update_ui_stat(self.controller_stat.smistatore_statistica(self.selected, self.anno))
            ...
            self.andamento_finanziario(self.controller_stat.smistatore_statistica(self.selected, self.anno))

    # Metodo che consente di tornare alla schermata precedente a quella appena visualizzata
    def show_back_click(self):
        self.view_back = listastatistiche.view.ViewListaStatistiche.ViewListaStatistiche()
        self.view_back.showMaximized()
        time.sleep(0.3)
        self.close()
